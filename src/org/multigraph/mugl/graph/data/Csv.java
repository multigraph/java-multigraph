...
public class Csv extends MuglBean {
    ...
    public static Csv parse( Element node, Element defaultsNode, org.multigraph.mugl.graph.Data parent ) throws Exception {
        final Csv csv = new Csv();
        
        setProperty("location", node, defaultsNode,
                    new PropertySetter() { public void set(String value, boolean isDefault) {
                        csv.setLocation( value, isDefault );
                    }});
        
        return csv;
    }
    public Element build(boolean includeAll) throws Exception {
    	Element element = new Element("csv");
